...
#### decoders ####
from .decoders.mlp_decoder import DecoderHead
from .decoders.simmim import SimMIM
...
class dformerv2(nn.Module):
    ...
    def forward_encoder(self, x, x_e):
        # rgb input
        # print(f"X BEFORE PATCH EMBED {x.size()}")
        x = self.patch_embed(x)

        x, mask = self.random_masking(x)

        # print(f"X AFTER PATCH EMBED {x.size()}")
        # depth input
        # x_e = x_e.unsqueeze(1)
    ...
    def random_masking(self, x):
        """
        Perform 4D random masking by REPLACING patches with self.mask_token.
        x: [N, H, W, D] (e.g., 256, 56, 56, 64)
        """
        ...
        # 1. Generate the binary mask (for the loss function)
        # 0 is keep, 1 is remove/mask
        mask = torch.ones([N, L], device=x.device)
        mask[:, :len_keep] = 0
        mask = torch.gather(mask, dim=1, index=ids_restore)
        # `mask` shape is [N, L] (e.g., 256, 3136)

        # 2. Create the 4D masked tensor (for the encoder)
        mask_4d = mask.reshape(N, H, W, 1).to(torch.bool)
        
        # Replace patches
        x_masked = torch.where(mask_4d, self.mask_token, x)
        # `x_masked` shape is [N, H, W, D] (e.g., 256, 56, 56, 64)
    ...
    def forward_loss(self, x, pred, mask):
        ...
        # pred = pred.reshape(pred.shape[0], -1, 4*4*3)
    ...
    def forward(self, x, x_e, knn=False):
        latent, mask = self.forward_encoder(x, x_e)
        # print(f"latent {latent.size()}")
        if knn: 
            return latent
