...
EVENT=/tmp/dinocular_lmdort_rope_dino_seed1_watch.event
LOG=/tmp/dinocular_lmdort_rope_dino_seed1_watch.log
...
while :; do
    ...
    if [ -z "$PREVIOUS" ]; then
        ...
        case "$STATE" in
            COMPLETED|FAILED|CANCELLED|TIMEOUT|OUT_OF_MEMORY|NODE_FAIL|PREEMPTED|BOOT_FAIL|DEADLINE|REVOKED|SPECIAL_EXIT)
                DETAIL=$(ssh -o BatchMode=yes -o ConnectTimeout=15 lmdort \
                    ...
                    printf \"PROGRESS\\\\n\"; sed -n \"1,260p\" \"$ROOT/outputs/campaign-seed1/rgb-dino-seed1-20260728a/rope/run/progress.json\" 2>/dev/null || true; \
                    printf \"STDOUT_TAIL\\\\n\"; tail -n 120 \"$ROOT/lmdort/logs/segment.$JOB_ID.out\" 2>/dev/null || true; \
                    printf \"STDERR_TAIL\\\\n\"; tail -n 120 \"$ROOT/lmdort/logs/segment.$JOB_ID.err\" 2>/dev/null || true; \
                    printf \"OUTPUT_FILES\\\\n\"; find \"$ROOT/outputs/campaign-seed1/rgb-dino-seed1-20260728a/rope/run\" -maxdepth 4 -type f -printf \"%s %p\\\\n\" | sort'" \
                    2>&1)
                {
                    printf '\nTERMINAL_DETAIL\n%s\n' "$DETAIL"
                } >> "$EVENT"
        ...
        herdr pane run "$TARGET" \
            "MATERIAL LMDORT ROPE DINOv2 SEED-ONE EVENT. Read $EVENT once, verify job $JOB_ID and the lmdort receipt/log paths it names, integrate only the changed technical state, and do not model-poll. Do not message Hannah from this worker route." \
            >> "$LOG" 2>&1 || true
        PREVIOUS=$CURRENT
    fi
